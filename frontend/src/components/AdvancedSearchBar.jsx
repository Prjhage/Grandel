...
const AdvancedSearchBar = () => {
    ...
    const renderMonth = (monthOffset) => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const isInRange = filters.startDate && filters.endDate && dateStr > filters.startDate && dateStr < filters.endDate;
    ...
    return (
        <>
            {/* Mobile Trigger */}
            <div className="mobile-search-trigger" onClick={() => setIsMobileModalOpen(true)}>
            ...
            <div className="advanced-search-bar desktop-only">
                                ...
                                if (navigator.geolocation) {
                                    ...
                                    navigator.geolocation.getCurrentPosition(
                                        (position) => {
                                            ...
                                            const newFilters = { ...filters, q: 'Current Location', lat: latitude, lng: longitude };
                ...
                >
                    ...
                    {isCalendarOpen && (
                        <div className="calendar-dropdown shadow-lg p-4 rounded-4 bg-white" onClick={e => e.stopPropagation()}>
                            <div className="calendar-main-container">
                                <button className="calendar-nav-btn nav-prev" onClick={() => setViewMonth(viewMonth - 1)}>
                                ...
                                <button className="calendar-nav-btn nav-next" onClick={() => setViewMonth(viewMonth + 1)}>
                                    <i className="fa-solid fa-chevron-right"></i>
                                </button>
                            </div>
                            <div className="d-flex justify-content-end mt-3 gap-3 align-items-center">
                                <button className="btn btn-link btn-sm text-muted text-decoration-none fw-600 p-0" onClick={() => setFilters(p => ({ ...p, startDate: '', endDate: '' }))}>Clear Dates</button>
                                <button className="btn btn-dark btn-sm px-4 rounded-pill fw-600" onClick={() => setIsCalendarOpen(false)}>Apply</button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                >
                    ...
                    {isGuestDropdownOpen && (
                        <div className="guest-picker-dropdown shadow-lg p-3 rounded-4 bg-white" onClick={e => e.stopPropagation()}>
                            <div className="d-flex justify-content-between align-items-center mb-3">
                                <span>Rooms</span>
                                <div className="picker-counter">
                                    <button onClick={() => handleFilterChange('rooms', Math.max(1, parseInt(filters.rooms) - 1))}>-</button>
                                    <span>{filters.rooms}</span>
                                    <button onClick={() => handleFilterChange('rooms', parseInt(filters.rooms) + 1)}>+</button>
                                </div>
                            </div>
                            <div className="d-flex justify-content-between align-items-center">
                                <span>Guests</span>
                                <div className="picker-counter">
                                    <button onClick={() => handleFilterChange('guests', Math.max(1, parseInt(filters.guests) - 1))}>-</button>
                                    <span>{filters.guests}</span>
                                    <button onClick={() => handleFilterChange('guests', parseInt(filters.guests) + 1)}>+</button>
                                </div>
                            </div>
                            <button className="btn btn-dark btn-sm w-100 mt-3" onClick={() => setIsGuestDropdownOpen(false)}>Done</button>
                        </div>
                    )}
                </div>

                {Object.values(filters).some(v => v !== '' && v !== '1') && (
                    <button className="btn-clear-inline" onClick={(e) => clearAllFilters(e)} title="Clear all filters">
